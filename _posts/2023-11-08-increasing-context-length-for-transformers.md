...
description: Self-attention is not very efficient. How can we make it better, without compromising performance?
date: 2023-11-08
htmlwidgets: true
...
Our evaluation metric for all models was next-token cross-entropy loss against a corpus of Shakespeare texts. Base experiment results are shown below.
...
In order to generalize to larger models, Gaussian attentions may need to be combined with other attention patterns, like global attention. More research is needed to fully understand its potential and shortcomings.

## Conclusion

Overall, our experiments showed that Gaussian attention has potential as a context-reduction algorithm for improving transformer efficiency. However, our experiments may not reflect the algorithm’s actual performance in real-world scenarios. Because we did not have the capacity to train a language model on the scale of BERT or GPT, we experimented only with much smaller models that processed much smaller contexts. As a resort, our experimental results may not extend to larger models. Additionally, due to limited training time, we did not train any of the models we used for more than 100 epochs—with more training time, it is possible that the base transformers may outperform the modified ones.

Today, algorithms for reducing context length in transformers remain an important research topic. With new developments in the field, we may soon be able to develop more powerful and more efficient transformers.
