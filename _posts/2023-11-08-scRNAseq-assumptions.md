...
# 			Binning:
# 				scGPT - binning of gene expression values, use bin as value, binning occurs after log1p and highly variable gene selection), MLM-style pretraining, uses non-zero genes
# 				scBERT - bin and embed (discretizes the continuous expression variables by binning, and converts them into 200-dimensional vectors, which are then used as token embeddings for the scBERT model.), MLM-style pretraining, uses ~16k genes w/ Performer
