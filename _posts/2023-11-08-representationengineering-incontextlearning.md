...
For example, <d-cite key = "zhou2022teaching"></d-cite> evaluates how LLM can become better at solving algorithmic problems through in-context learning, a task that LLM traditionally struggles at. 
...
Relatedly, there have been a recent surge in research related to model knowledge editing, including <d-cite key = "meng2023massediting"></d-cite>, <d-cite key = "zhong2023mquake"></d-cite>, and <d-cite key = "hernandez2023inspecting"></d-cite> that demonstrate different methods for locating and editing factual associations. Other work, including <d-cite key="shao2023gold"></d-cite> and <d-cite key="belrose2023leace"></d-cite>, have shown results on concept erasures. 

<d-cite key="li2023inferencetime"></d-cite> proposes Inference-Time Intervention, where they find directions of causal influence on "truthfulness" data and increase the activations along that direction to increase truthfulness, scoring better on the TruthfulQA dataset. 

# Experiment Setup

## Datasets

We adopt the following 6 datasets on binary classification tasks of the total 26 dataset used by <d-cite key="min2022rethinking"></d-cite> to evaluate in-context learning. We use `rotten_tomatoes`, `ade_corpus_v2-classification`, `sick`, and `XXX` datasets. Following <d-cite key="min2022rethinking"></d-cite>, we only use the test set to avoid potential cross-contamination with the data that the model is pretrained on.  reserve `k=64` examples in the test for few-shot training, and the rest are used for testing. 
