...
{% include figure.html path="/assets/img/2023-11-09-autodecoders/tsne_autodecoder.png" class="img-fluid" caption="The latent space learned by the autodecoder, also visualized through a 2-dimensional t-SNE plot. We again see consistency, but notice that the clusters are more compact. While distance between clusters in  t-SNE plots do not have a definite meaning, this could potentially imply that features of shapes, rather than the shapes themselves, are better learned, as different digits share similar features (curves, straight lines, etc)."%}
