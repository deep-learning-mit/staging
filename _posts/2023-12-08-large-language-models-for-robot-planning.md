...
Due to ease of access, we decided to use ChatGPT’s GPT4-Vision functionality to run our experiments. A more comprehensive ablation may analyze these experiments across a wider range of VLMs.
...
Raw Generation: Image + generic prompt
...
Prompt 1 + describe each object in the scene
...
Prompt 2 + describe the target task
...
Prompt 3 + explain object relations in detail
...
Since several different PDDL domains can be used to represent the same set of actions and predicates, the task of generating PDDL files is quite subjective. Since generating PDDL tasks is an often-tedious task that humans must do themselves to represent any given domain, we evaluate each VLM output based on its similarity to real PDDL domains handcrafted manually. After asking the VLM to generate a PDDL file, both the **target** and the **generated** domain descriptions are embedded using the Longformer: Long Document Transformer [CITE] model, before finally computing the cosine similarity between the two embeddings. We decided to use Longformer, instead a more traditional model like LLAMA or BERT, due to the long context-length afforded to us by the model; a more comprehensive study would require studying these effects on a wider range of embedding models to more accurately understand the text-to-text similarity between the two domains. 
...
Aside from this, PDDL’s structure also provides several keywords that are commonly used by all PDDL files, as detailed below. In order to handle these, we decided to simply remove all instances of these words from both the target and the generated PDDL files, in order to mitigate the effect of the similarity between these tokens.

Figure ?: [Add a code block with a caption here explaining the list of words that we are masking]

## Results

After experimenting on a wide range of complex environments with various prompting strategies, it seems that VLMs perform quite well for the task of generating PDDLs from image and text conditioning. We measured the similarity of the ground truth PDDL file with each image and experiment’s generated PDDL file. To quantitatively measure similarity, we used the cosine similarity metric on the embeddings of the masked pieces of text using Longformer [CITATION]. We did not use BERT [CITATION] due to the context length not being long enough for our PDDL files. Recall that we masked out certain frequently appearing words in PDDL files so as to not inflate the similarity due to this commonality. 

The exact generated PDDL files can be found at https://github.com/anirudhv27/final-project-dl/tree/main/experiments. 
...
Caption: The correspondence between the inputted image and the generated predicates. Note that the predicates are not descriptive of just the current state, but also are descriptive of the later potential states based on the inferred task. 

[Create image which has left side the “cut” 6 predicates: “(plate-empty), (bowl-empty), (bagel-on-plate), (bagel-on-bowl), (knife-on-plate), (bagel-cut)”, and the right side is the image itself. Make arrows from the left side to the corresponding correct part of the image on the right side.]
...
Our work analyzes an important question in the 
Our methodology proved to be successful on the 

A more comprehensive ablation may analyze these experiments across a wider range of VLMs.